...
const validateTaskFields = [
    body('title').notEmpty().withMessage('Title is required.').bail().isLength({ min: 5}).withMessage('Title must be at least 5 characters long.'),
    body('description').notEmpty().withMessage('Description is required.').bail().isLength({ min: 10}).withMessage('Description must be at least 10 characters long.is required.')
